...
First Version: 1.0 //Old not recommended.
Second Version: 1.1 //Old not recommended.
Third Version 1.2 // New and recommended!
...
CMD:dmevent(playerid, params[])
{
    if(IsPlayerAdmin(playerid))
    {
        if(EventOpened[playerid] == 0)
        {
            ...
            if(weapon < 0 || weapon > 46) return SCM(playerid,-1,"{f00f00}[ERROR]: {ffffff}Invalid Weapon ID!");
        ...
        else
        {
            ...
            {
                SCMTA(-1,"{800080}[EVENT INFO] {fef65b}An Administrator has closed the DM Event, and started it!");
...
CMD:fire(playerid,params[])
{
    ...
    format(string,sizeof(string),"{800080}[EVENT FIRE] {Fef65b}%s has been fired from the DM Event! {f00f00}[REASON: %s]",tname, params);
    SCMTA(-1,string);
    InEvent[ID] = 0;
    SpawnPlayer(ID);
    return 1;
...
CMD:joindm(playerid,params[])
{
    ...
    if(EventOpened[playerid] == 0) return SendClientMessage(playerid, -1,"{f00f00}[ERROR]: {FFFFFF}there isn't any DM Event at the moment!");
    new Random = random(sizeof(RandomEventSpawns)), name[MAX_PLAYER_NAME], id, string[128];
    ...
    InEvent[playerid] = 1;
	EventOpened[playerid] = 1;
    format(string,sizeof(string),"{2faced}JOINED: {fef65b}%s(%i) {800080}has joined the DM event!",name,id);
    SendClientMessageToAll(-1,string);
    SendClientMessage(playerid, -1,"{800080}[INFO] {fef65b}Type {f00f00}/leavedm {fef65b}if you want to leave");
...
CMD:leavedm(playerid,params[])
{
  ...
  {
	 if(EventStarted[playerid] == 1) return SendClientMessage(playerid, -1,"[ERROR]: {FFFFFF}You cannot leave when the DM is already started!");
     new
	 ...
	 InEvent[playerid] = 0;
	 format(string,sizeof(string),"{2faced}LEFT: {fef65b}%s(%i) {800080}has left the DM event! {f00f00}(/leavedm)",name,id);
	 SCMTA(-1, string);
  }
  else
  {
  SendClientMessage(playerid, -1,"{f00f00}[ERROR]: {ffffff}You aren't in any DM event to leave!");
  }
  return 1;
}
...
{
    SendClientMessage(playerid, -1, "{fef65b}DM Event System Loaded!");
    SendClientMessage(playerid, -1, "{95fa7f}it's Created by RxErT!");
    SendClientMessage(playerid, -1, "{0099cc}Type /dmcmds & /updates to get available commands!");
    return 1;
}
...
CMD:dmcmds(playerid, params[])
{
    if(IsPlayerAdmin(playerid))
    {
        ...
        strcat(string, "{fef65b}/dmevent {1ffdc4}-> {fd1f58}again type to close and start the event!\n");
        strcat(string, "{fef65b}/fire{1ffdc4} -> {fd1f58}to fire/kick someone from the DM event!\n");
        strcat(string, "{fef65b}/updates{1ffdc4} -> {fd1f58}to see what's new!\n\n");
...
{
   new string[500];
   strcat(string, "{fef65b}1. {fd1f58}Added a {1ffdc4}`text` {fd1f58}for the reason of the /fire!\n");
   strcat(string, "{fef65b}2. {fd1f58}Optimized the code and the language.\n");
   strcat(string, "{fef65b}3. {fd1f58}Combined /startdm with /dmevent, so the event will start and close just when you type /dmevent again!\n\n");
   ShowPlayerDialog(playerid, 1884, DIALOG_STYLE_MSGBOX,"News - Updates", string, "Ok", "");
   return 1;
}
